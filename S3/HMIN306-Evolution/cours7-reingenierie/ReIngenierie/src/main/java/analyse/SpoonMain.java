...
//		environment.setComplianceLevel(0); // sets the java compliance level.
		...
		Collection<CtType<?>> ctTypeList = model.getAllTypes();
		System.out.println("Total Invocation : " + model.getElements(new TypeFilter<CtAbstractInvocation>(CtAbstractInvocation.class)).size());
		for(CtType<?> ctType : ctTypeList) {
			if(!ctType.isPrimitive()) {
				List<CtAbstractInvocation> ctAbstractInvocationList = ctType.getElements(new TypeFilter<CtAbstractInvocation>(CtAbstractInvocation.class)); 
				for(CtType<?> ctType2 : ctTypeList) {
					if(!ctType2.isPrimitive()) {
