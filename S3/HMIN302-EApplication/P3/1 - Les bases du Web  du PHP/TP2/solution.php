...
function accronym(string $string) : string {
    ...
    return '<p title="' . $string . ' ">' . strtoupper($accronym) . '</p>';
}
...
function syracuse(int $int) : string
{
    ...
    // Test
    if (!is_int($int)) {
        return '"' . $int . '" n\'est pas un nombre entier !';
    } elseif ($int <= 0) {
        return '"' . $int . '" n\'est pas un nombre entier positif !';
    }
...
<h1><?= $title ?></h1>
    ...
    <article>
        ...
        <p>Ouvrir le ficher "chuck.json", et sortir aléatoirment 5 Chuck Norris facts.</p>
