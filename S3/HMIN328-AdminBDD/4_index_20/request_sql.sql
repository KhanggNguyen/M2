...
/* ROWID correspond à l'adresse de la ligne. C'est un identifiant unique de la ligne  */
/* ROWNUM correspond à la numéro de la ligne du résultat d'une requete */

/* q2 */

/* Les indexes présentes dans mon schéma utilisateur sont les clés primaires des tables */

/* q3 */
/* ces indexes sont uniques car chaque tuples ont une valeur unique puisque c'est aussi la clé primaire de la table */
/* ce sont les indexes denses puisque tous les valeurs de clé sont représentés */
/* par ex table commune */
SELECT COUNT(ROWID) from commune;
SELECT COUNT(CODEINSEE) FROM commune;

/* q4 */
/* hauteur de la taille de l'index de la table commune est 1 */
SELECT blevel FROM user_indexes WHERE table_name = 'COMMUNE';
