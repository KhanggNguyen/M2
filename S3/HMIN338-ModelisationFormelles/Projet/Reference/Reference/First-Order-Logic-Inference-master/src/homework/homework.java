...
public class homework {
	...
	public static Literal getLiterals(String sentence) {
		...
		Literal.variable.setElementAt(Literal.variable.get(Literal.variable.size() - 1).substring(0,
				Literal.variable.get(Literal.variable.size() - 1).length() - 1), Literal.variable.size() - 1);
	...
	public static variable_unify doUnification(expr temp1, expr temp2, variable_unify mapping) {
		...
		if (i == 1 && j == 1) {
			...
			try {
				x = Character.isUpperCase(first.value.get(0).charAt(0)) ? new constant(first.value.get(0))
						: new var(first.value.get(0));
				y = Character.isUpperCase(second.value.get(0).charAt(0)) ? new constant(second.value.get(0))
						: new var(second.value.get(0));
			} catch (Exception e) {
		...
		for (int i = 0; i < clause1.size(); i++) {
			for (int j = 0; j < clause2.size(); j++) {
				...
				if (compareLiterals(literal1, literal2)) {
					...
					// Character.isLowerCase(mapping.variable_val.get(key).charAt(0)))
					// {
					// mapping.result_unify = true;
					// }
					// }
	...
	public static boolean equals(Vector<Literal> clause1, Vector<Literal> clause2) {
		...
		try {
			for (int i = 0; i < clause1.size(); i++) {
				if (clause1.elementAt(i).negation == clause2.elementAt(i).negation
						&& clause1.elementAt(i).name.compareTo(clause2.elementAt(i).name) == 0) {
					for (int j = 0; j < clause1.elementAt(i).variable.size(); j++) {
						if (clause1.elementAt(i).variable.size() != clause2.elementAt(i).variable.size())
							return false;
						if (clause1.elementAt(i).variable.elementAt(j)
								.compareTo(clause2.elementAt(i).variable.elementAt(j)) != 0) {
							return false;
						}
					}
				} else {
	...
	public static Vector<Literal> convertToPostFix(Vector<Literal> exp) {
		...
		for (Literal Literal : exp) {
			...
			if (Literal.category == 5) {
				...
				while (!stack.empty()
						&& precedence(Literal.name.charAt(0)) <= precedence(stack.lastElement().name.charAt(0))) {
